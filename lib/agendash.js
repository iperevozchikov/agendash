...
module.exports = function(agenda, options) {
    ...
    try {
      ...
      .toArray((err, jobs) => {
        ...
        async.series(jobs.map(job => done => {
          agenda.create(job.name, job.data).save().then(
            newJob => done(null, newJob),
            err => done(err, null)
          );
        }), (err, results) => {
  ...
  const deleteJobs = (jobIds, callback) => {
    ...
    try {
      ...
      agenda.cancel({_id: {$in: jobIds.map(jobId => collection.s.pkFactory(jobId))}}).then(
        () => callback(),
        err => callback('Jobs not deleted: ' + err.message)
      );
    } catch (err) {
      callback(err.message);
    }
  ...
  const createJob = (jobName, jobSchedule, jobRepeatEvery, jobData, callback) => {
    ...
    try {
      ...
      job.save().then(
        () => callback(),
        err => callback('Jobs not created: ' + err.message)
      );
    } catch (err) {
      callback(err.message);
    }
